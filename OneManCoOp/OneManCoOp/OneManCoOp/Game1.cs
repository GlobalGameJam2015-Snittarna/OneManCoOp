    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        public void DrawUi()
        {
            switch(gameState)
            {
                ...
                if (tutorialCount <= 128*6)
                {
                    if (!GamePad.GetState(PlayerIndex.One).IsConnected) spriteBatch.DrawString(TextureManager.font, "A, S, D to move. Hold W too stand on a \"ghost\". \nR to deplete your time and place a \"ghost\"  \nJump with space \nAll buttons has to be pressed down in a room by a \nplayer or \"ghost\" to open the trapdoor", new Vector2(Camera.Position.X - 400 + 32, Camera.Position.Y - 300), Color.Green);
